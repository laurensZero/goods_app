...
-- ============================================================
-- 9. OTA Bundles — Web Bundle OTA 更新分发（替代 GitHub Pages）
-- ============================================================
...
CREATE TABLE IF NOT EXISTS ota_bundles (
  ...
  type          TEXT NOT NULL DEFAULT 'web_bundle'
                CHECK (type IN ('web_bundle')),
...
CREATE INDEX IF NOT EXISTS idx_ota_bundles_channel_published
  ON ota_bundles(channel, published_at DESC);

-- RLS: anon/public 可读（客户端查询最新版本），service_role 全权（CI 写入）
ALTER TABLE ota_bundles ENABLE ROW LEVEL SECURITY;

CREATE POLICY "ota_bundles_public_select" ON ota_bundles FOR SELECT USING (true);

CREATE POLICY "ota_bundles_service_all" ON ota_bundles FOR ALL
  USING (auth.role() = 'service_role')
  WITH CHECK (auth.role() = 'service_role');

GRANT SELECT ON ota_bundles TO anon, authenticated;

-- Storage bucket: ota-bundles（公开读取，service_role 写入）
INSERT INTO storage.buckets (id, name, public)
VALUES ('ota-bundles', 'ota-bundles', true)
ON CONFLICT (id) DO NOTHING;

DROP POLICY IF EXISTS "ota_bundles_public_read" ON storage.objects;
DROP POLICY IF EXISTS "ota_bundles_service_all" ON storage.objects;

CREATE POLICY "ota_bundles_public_read" ON storage.objects
  FOR SELECT USING (bucket_id = 'ota-bundles');

CREATE POLICY "ota_bundles_service_all" ON storage.objects
  FOR ALL USING (bucket_id = 'ota-bundles' AND auth.role() = 'service_role')
  WITH CHECK (bucket_id = 'ota-bundles' AND auth.role() = 'service_role');
