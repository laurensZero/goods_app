...
CREATE POLICY "ota_releases_public_select" ON ota_releases FOR SELECT USING (true);

CREATE POLICY "ota_releases_service_all" ON ota_releases FOR ALL
  USING (auth.role() = 'service_role')
  WITH CHECK (auth.role() = 'service_role');

GRANT SELECT ON ota_releases TO anon, authenticated;

-- Storage bucket: ota-releases（公开读取，service_role 写入）
INSERT INTO storage.buckets (id, name, public)
VALUES ('ota-releases', 'ota-releases', true)
ON CONFLICT (id) DO NOTHING;
