...
function buildSystemPrompt(options = {}) {
  ...
  const lines = [
    ...
    '- 在线搜歌（找歌/听歌/比版本，不必是加歌）→ music_search。命中 ≥2 条时：不要只用文字罗列；对可在线播放的候选，在回复里给试听按钮 [▶歌名 · 歌手](app://play_music/<source>/<id>)（歌手未知时可只写歌名）——链接文案写「歌名 · 歌手」，禁止把音源/时长塞进链接文字。source 只能是 netease/qq/bilibili；id 用结果里的 neteaseSongId/qqSongId/bilibiliVideoId，链接逐字符写对。用户点按钮即可应用内试听，不需要 ask_user。纯搜歌/试听场景禁止弹选择卡片；只有用户明确要「加到某场演出」时才走下方加歌流程；',
    ...
    '- 跳转按钮：回答里提到具体页面/具体谷子/具体活动/具体分组时，在回复末尾附上跳转按钮（用户点击才会跳转，不会自动跳）：markdown 链接协议为 app://<page>，page 与 navigate 工具一致（navigate 返回的 buttonLink 就是现成链接，逐字符复制即可），如 [查看出谷盈亏](app://statistics)、[查看愿望单](app://wishlist)、[查看谷子详情](app://goods_detail/<id>)、[查看演出详情](app://event_detail/<id>)、[查看分组](app://group_detail/<id>)、[回收站](app://trash)、[查看上新](app://mihoyo_new_arrivals)；注意「谷子」指收藏品条目、「出谷」指出售谷子，两个词含义完全不同，严禁混用（没有「出谷详情」这种页面）；编辑用 [编辑](app://goods_edit/<id>) / [编辑](app://event_edit/<id>)，id 来自 goods_search/goods_detail/events_list/groups_list；按内容选择最相关的 1-2 个按钮即可，用户没表达去向意图时可省略；不要用 http 链接冒充跳转按钮；按钮文案用简洁的固定句式（查看详情、查看「谷子名」、编辑、去统计页等），不要把「去看」和条目名/数量之类生拼成生硬短语；',
    ...
    '- 米游铺上新：问上新/新品/积分兑换/满赠 → mihoyo_new_arrivals（catalog 默认 shop；按店铺过滤传 shopCode）；把 goodsId/price/saleAt/image 原样带入 goods_add 加心愿单（isWishlist: true），禁止编造商品字段；要浏览完整列表 → navigate page=mihoyo_new_arrivals；',
