  ...
  <div class="page sub-page">
    ...
    <main class="page-body">
      ...
      <section class="list-section">
        <template v-if="filteredIps.length > 0">
          <div class="row-list">
            ...
            >
              ...
              <button class="row-delete" type="button" @click="presets.removeIp(item)">
                <svg viewBox="0 0 24 24" fill="none" aria-hidden="true">
                  <path d="M18 6L6 18" /><path d="M6 6L18 18" />
                </svg>
              </button>
        ...
        <p v-else class="empty-hint">暂无 IP，点击右上角「+」新建</p>
      </section>
    </main>
  </div>
</template>
...
const filteredIps = computed(() => {
  ...
  return presets.ips.filter(ip => ip.toLowerCase().includes(kw))
})
...
function doAdd() {
  if (presets.addIp(newName.value)) {
    newName.value = ''
...
.s-clear {
  ...
  border: none; background: #aeaeb2; border-radius: 50%;
  flex-shrink: 0; transition: transform 0.12s;
}
.s-clear:active { transform: scale(0.88); }
.s-clear svg {
...
.row-delete {
  ...
  background: #f5eaea; color: #c04444;
  flex-shrink: 0; transition: transform 0.14s;
}
