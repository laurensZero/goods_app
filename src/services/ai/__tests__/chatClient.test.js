...
describe('chatClient', () => {
  ...
  it('纯文本回答：一次请求完成，不带工具步骤', async () => {
    CapacitorHttp.fetch.mockResolvedValueOnce(completion('你好，这是回答'))
    ...
    const [url, options] = CapacitorHttp.fetch.mock.calls[0]
  ...
  it('工具调用循环：执行工具、回填 tool 消息、二轮拿到最终回答', async () => {
    ...
    expect(CapacitorHttp.fetch).toHaveBeenCalledTimes(2)
    sentPayloads.push(CapacitorHttp.fetch.mock.calls[0][1].data)
    sentPayloads.push(CapacitorHttp.fetch.mock.calls[1][1].data)
  ...
  it('工具执行失败时把错误交给模型继续回答', async () => {
    ...
    // 工具错误以 {error} 形式回填
    const toolMessage = CapacitorHttp.fetch.mock.calls[1][1].data.messages.at(-1)
  ...
  it('Web 端 dev 下经 /ai-proxy 转发，带 Content-Type 与 x-ai-target', async () => {
    ...
    try {
      ...
      expect(CapacitorHttp.fetch).not.toHaveBeenCalled()
  ...
  it('超过工具轮数上限后强制发起无工具的收尾请求', async () => {
    ...
    for (let i = 0; i < 6; i += 1) {
      CapacitorHttp.fetch.mockResolvedValueOnce(completion(null, [
        { id: `call-${i}`, function: { name: 'goods_search', arguments: '{}' } }
      ]))
    }
    // 第 7 次是收尾请求（无 tools）返回文本
    CapacitorHttp.fetch.mockResolvedValueOnce(completion('不再调用工具了'))
    ...
    expect(CapacitorHttp.fetch).toHaveBeenCalledTimes(7)
    // 收尾请求不带 tools 参数
    const lastPayload = CapacitorHttp.fetch.mock.calls[6][1].data
