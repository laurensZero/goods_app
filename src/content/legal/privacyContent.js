...
| 问卷 | 您提交的问卷答案 | 改进产品 |
| 分享链接 | 您选择公开分享的条目信息 | 生成可供他人导入的分享内容 |
...
- **您明确同意或指示**：如开启云同步、创建分享链接；使用米游铺功能时，相当于指示本应用将 Cookie 及必要请求参数发送至米游铺/米哈游接口
- **实现功能所必需的技术服务**：如云数据库/存储服务商，在必要范围内处理您授权的数据
- **法律法规或行政机关要求**：在适用法律要求的范围内披露

应用内的音乐、汇率等第三方内容服务，由对应服务商按其政策处理您发起请求时产生的数据。米游铺相关数据由米哈游/米游铺按其政策处理。
...
- **米游铺 / 米哈游接口**：可选的订单与购物车导入、有货监控、下单辅助等；相关请求在您使用功能时携带本机保存的 Cookie，由米游铺服务器处理
...
| 問卷 | 您提交的問卷答案 | 改進產品 |
| 分享連結 | 您選擇公開分享的條目資訊 | 生成可供他人匯入的分享內容 |
...
- **您明確同意或指示**：如開啟雲端同步、建立分享連結；使用米游鋪功能時，相當於指示本應用將 Cookie 及必要請求參數發送至米游鋪/米哈遊介面
- **實現功能所必需的技術服務**：如雲端資料庫/儲存服務商，在必要範圍內處理您授權的資料
- **法律法規或行政機關要求**：在適用法律要求的範圍內揭露

應用內的音樂、匯率等第三方內容服務，由對應服務商按其政策處理您發起請求時產生的資料。米游鋪相關資料由米哈遊/米游鋪按其政策處理。
...
- **米游鋪 / 米哈遊介面**：可選的訂單與購物車匯入、有貨監控、下單輔助等；相關請求在您使用功能時攜帶本機保存的 Cookie，由米游鋪伺服器處理
...
| Share links | Item details you choose to publish | Generate content others can import |
...
### 1.4 Other third-party APIs (user-initiated)
...
- You control on-device data: export backups, or delete via trash/settings.
...
- **With your clear consent or instruction**: e.g., enabling cloud sync, creating share links; using miHoYo Shop features instructs the App to send the Cookie and necessary request parameters to miHoYo Shop / miHoYo APIs
...
Third-party content services in the App (music, exchange rates, etc.) process request-related data under their own policies. miHoYo Shop-related data is processed by miHoYo / miHoYo Shop under their policies.
...
- **miHoYo Shop / miHoYo APIs**: optional order/cart import, stock monitoring, checkout assistance; requests carry the locally saved Cookie when you use these features and are processed by miHoYo Shop servers
...
Detailed rules of each provider are governed by their official policies.
...
| アンケート | ご回答内容 | 製品改善 |
| 公開リンク | 公開を選択した項目情報 | 他者が取り込める共有内容の生成 |
...
- 端末データはお客様の管理下にあり、書き出し・削除（ゴミ箱/設定）が可能です。
...
- **お客様の明確な同意または指示**：クラウド同期の有効化、共有リンク作成など。miHoYo Shop 機能利用は、Cookie と必要パラメータを miHoYo Shop / miHoYo API へ送信する指示を含みます
- **機能提供に必要な技術サービス**：クラウド DB/ストレージ等、許諾範囲での処理
- **法令または行政機関の要求**：適用法が要求する範囲での開示

アプリ内の音楽・為替等の第三者コンテンツは、各社の方針に従いリクエスト関連データを処理します。miHoYo Shop 関連データは miHoYo / miHoYo Shop の方針に従い処理されます。
...
- **miHoYo Shop / miHoYo API**：任意の注文/カート取り込み、在庫監視、注文補助。機能利用時に端末保存の Cookie を付与し、miHoYo Shop サーバーで処理
- **アプリ更新配布**：公式更新パッケージの取得
- **プラットフォーム機能**：Android/iOS 等の権限と API（通知、ファイル、クリップボード等）。該当機能利用時のみ呼び出し

各社サービスの詳細は各公式ポリシーに従います。
...
| 설문 | 제출한 답변 | 제품 개선 |
| 공유 링크 | 공개로 선택한 항목 정보 | 타인이 가져올 수 있는 공유 내용 생성 |
...
- 기기 데이터는 귀하가 통제합니다: 내보내기 백업, 휴지통/설정에서 삭제 가능.
...
- **귀하의 명시적 동의 또는 지시**: 클라우드 동기화 켜기, 공유 링크 생성 등. miHoYo Shop 기능 사용은 Cookie와 필요한 요청 파라미터를 miHoYo Shop / miHoYo API로 보내는 지시를 포함합니다
- **기능 제공에 필요한 기술 서비스**: 클라우드 DB/스토리지 등, 허가 범위 내 처리
- **법령 또는 행정기관 요구**: 적용 법이 요구하는 범위에서 제공

앱 내 음악·환율 등 제3자 콘텐츠는 각 서비스 정책에 따라 요청 관련 데이터를 처리합니다. miHoYo Shop 관련 데이터는 miHoYo / miHoYo Shop 정책에 따라 처리됩니다.
...
- **miHoYo Shop / miHoYo API**: 선택적 주문/장바구니 가져오기, 재고 모니터링, 주문 보조. 기능 사용 시 기기 저장 Cookie를 실어 miHoYo Shop 서버에서 처리
- **앱 업데이트 배포**: 공식 업데이트 패키지
- **플랫폼 기능**: Android/iOS 등의 권한·API(알림, 파일, 클립보드 등). 해당 기능 사용 시에만 호출

각 제공자의 상세 규칙은 해당 공식 정책을 따릅니다.
