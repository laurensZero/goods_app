...
/**
 * 米游铺商品搜索引擎（角色感知 + 关键词兜底 + 分页 + 款式封面增强）。
 * 收藏导入（ImportView）与自助下单（Checkout）共用同一套搜索逻辑。
 */
export function useMihoyoGoodsSearch(options = {}) {
  const { t } = useI18n()
  const presets = usePresetsStore()
  const scrollRootSelector = options.scrollRootSelector || '.import-page .page-body'
  ...
  /**
   * 标记选中搜索项（设置高亮与款式提示词）。消费者在调用后自行决定如何处理选中项
   * （收藏导入 → 解析 URL；自助下单 → 加入订单）。
   */
