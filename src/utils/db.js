...
export async function getItems() {
  ...
  if (IS_NATIVE) {
    ...
    rows = _webQuery('SELECT id,name,category,ip,characters,storageLocation,variant,price,acquiredAt,image,note,quantity,points FROM goods ORDER BY rowid DESC')
  }
  return rows.map(r => ({
    ...r,
...
export async function addItem(item) {
  const { id, name = '', category = '', ip = '', characters = [], storageLocation = '', variant = '', price = '', acquiredAt = '', image = '', note = '', quantity = 1, points } = item
  const charsStr = JSON.stringify(Array.isArray(characters) ? characters : [])
  const qty = Math.max(1, Number(quantity) || 1)
  const pts = points != null && points !== '' ? Number(points) : null
  const SQL = 'INSERT OR REPLACE INTO goods (id,name,category,ip,characters,storageLocation,variant,price,acquiredAt,image,note,quantity,points) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)'
  const p = [id, name, category, ip, charsStr, storageLocation, variant, price, acquiredAt, image, note, qty, pts]
  if (IS_NATIVE) {
    if (!_nativeDb) return
...
export async function saveItems(items) {
  if (IS_NATIVE) {
    ...
    for (const item of items) {
      ...
      const pts = points != null && points !== '' ? Number(points) : null
      stmts.push({
        statement: 'INSERT INTO goods (id,name,category,ip,characters,storageLocation,variant,price,acquiredAt,image,note,quantity,points) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)',
        values: [id, name, category, ip, charsStr, storageLocation, variant, price, acquiredAt, image, note, qty, pts]
      })
    }
    ...
    for (const item of items) {
      ...
      const pts = points != null && points !== '' ? Number(points) : null
      _sqlDb.run(
        'INSERT INTO goods (id,name,category,ip,characters,storageLocation,variant,price,acquiredAt,image,note,quantity,points) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)',
        [id, name, category, ip, charsStr, storageLocation, variant, price, acquiredAt, image, note, qty, pts]
      )
    }
    await _saveBinaryToIDB(_sqlDb)
  }
